...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ✅ Replace DayPilot Pro Scheduler with Syncfusion Scheduler in scheduler-logic.js
  ...
  function initScheduler() {
    ...
    const scheduler = new ej.schedule.Schedule({
      ...
      eventSettings: {
        dataSource: fakeData,
        fields: {
          subject: { name: "Subject", title: "Khách đặt" },
      ...
      popupOpen: function (args) {
        args.cancel = true;
        const { startTime, endTime, resource } = args.data;
        const room = rooms.find((r) => r.Id === resource);

        const modal = document.getElementById("bookingModal");
        if (!modal) return;

        modal.querySelector("input[name='room']").value = room?.Name || "";
        modal.querySelector("#roomTypeSelect").value = room?.Type || "";
        modal.querySelector("input[name='start']").value =
          startTime?.toISOString().slice(0, 16) || "";
        modal.querySelector("input[name='end']").value =
        ...
        form.onsubmit = function (e) {
          ...
          scheduler.addEvent({
            ...
            RoomId:
              rooms.find((r) => r.Name === booking.room)?.Id || rooms[0].Id,
            Name: booking.name,
    ...
    datePicker?.addEventListener("change", function () {
      scheduler.selectedDate = new Date(this.value);
      scheduler.dataBind();
    });

    viewModeSelect?.addEventListener("change", function () {
      const mode = this.value;
      if (mode === "day") scheduler.currentView = "TimelineDay";
      if (mode === "week") scheduler.currentView = "TimelineWeek";
      if (mode === "month") scheduler.currentView = "TimelineMonth";
    });

    window.schedulerInstance = scheduler;
